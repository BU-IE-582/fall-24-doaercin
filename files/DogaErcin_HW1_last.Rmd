...
```{r}
model_q2 <- lm(y3_new ~ df_new[,1] + I(df_new[,2]^2)  + df_new[,2] + df_new[,3] , df_new)
summary(model_q2)
...
y1<-c(img_new[[1]])
regression1 <-lm(y1~., df_test)
summary(regression1)

y2<-c(img_new[[2]])
regression2 <-lm(y2~., df_test)
summary(regression2)

y3<-c(img_new[[3]])
regression3 <-lm(y3~., df_test)
summary(regression3)


y4<-c(img_new[[4]])
regression4 <-lm(y4~., df_test)
summary(regression4)
```
